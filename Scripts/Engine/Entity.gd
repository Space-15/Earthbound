...
func damage_loop():
	...
	for area in $hitbox.get_overlapping_areas():
		for player in get_tree().get_nodes_in_group("actors"):
			 player.connect("player_hurt",self, "take_dmg")
			 connect("player_dead",self, "game_over")
			
		  
		var body = area.get_parent()
		if hitstun == 0 and body.get("DAMAGE") != null and body.get("TYPE") != TYPE:
			if TYPE == "PLAYER":
				emit_signal("player_hurt",body.get("DAMAGE"))
